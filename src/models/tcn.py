...
class _TemporalBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        target_len = min(out.shape[-1], res.shape[-1], x.shape[-1])
        out = out[:, :, -target_len:]
        res = res[:, :, -target_len:]
...
class _TCNNet(nn.Module):
    def __init__(self, input_dim: int, channels: list[int], kernel_size: int, dropout: float, task: str):
        super().__init__()
        layers = []
        in_ch = input_dim
...
class TCNModel(BaseModel):
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.channels = list(self.config.get("channels", [32, 32, 32]))
    ...
    def _to_2d(self, X: pd.DataFrame | np.ndarray) -> np.ndarray:
        ...
        if arr.ndim != 2:
            raise ValueError(f"TCNModel expects 2D input, got shape={arr.shape}")
        return arr.astype(np.float32, copy=False)

    def _build_sequences(self, X_scaled: np.ndarray, y: np.ndarray | None = None, prefix: np.ndarray | None = None):
    ...
    @classmethod
    def load(cls, path: str | Path) -> "TCNModel":
        ...
        obj._model = _TCNNet(
            ...
            channels=list(obj.config.get("channels", [32, 32, 32])),
